...
export async function POST(req: NextRequest) {
  ...
  const systemPrompt = isGeneralEssay
    ? 'You are an experienced writing tutor giving direct, specific feedback on a ' +
      ...
      '"scoreRationale": "one short sentence explaining the score", ' +
  ...
  // ---- 4. Record usage for free users only, but save history for everyone. ----
  if (!isPaid) {
    await supabase
      .from('admitly_users')
  ...
  const { data: savedEssay } = await supabase
    .from('admitly_essays')
    .insert({
      email,
      essay_type: essayType || 'Common App personal statement',
  ...
  return NextResponse.json(
    {
      feedback,
      essayId: savedEssay?.id ?? null,
      remaining: isPaid ? null : remainingCount - 1,
      isPaid,
    },
    { headers }
  );
